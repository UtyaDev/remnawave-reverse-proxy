...
add_antizapret_config() {
    ...
    # DNS modification
    config_json=$(echo "$config_json" | jq '.dns = {
        "servers": [
          {
            "port": 53,
            "address": "77.88.8.8",
    ...
    local alternative_ip=$(grep "ALTERNATIVE_IP" "$antizapret_compose_file" | cut -d'=' -f2)
    local alternative_fake_ip=$(grep "ALTERNATIVE_FAKE_IP" "$antizapret_compose_file" | cut -d'=' -f2)
        ...
        '
{
  "dns": {
    "servers": [
      { "address": "localhost", "domains": [ $domain ], "skipFallback": true },
      { "address": "https://1.1.1.1/dns-query", "domains": [ $domain ], "skipFallback": true },
...
main() {
    ...
    local ip_list=()
    ip_list+=("\$ip1" "\$ip2" "fd00:18::/111") # These are from original config, always include them.
    ip_list+=("\${additional_ips[@]}")
